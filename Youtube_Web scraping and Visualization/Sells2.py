...
def getSells():
    ...
    for t in transactiontypes:
        for i in range(NumPages):
            url = f"https://www.insidearbitrage.com/insider-sales/?date=&pagenum={i+1}"
            df = pd.read_html(url)
            df = df[2]
            columns = df.iloc[0]
            df.columns = columns
            df.drop(df.columns[1],axis=1,inplace=True) #axis 1 chooses the columns
            df = df[1:]
            if t == 'buying':
                df['Type'] = "buy"
            else:
